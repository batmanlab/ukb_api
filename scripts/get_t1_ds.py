...
with open("/ocean/projects/asc170022p/shared/Data/ukBiobank/datalad_dataset/code/remaining_brain_subjects.txt", "r") as fp:
    lines = fp.readlines()
    ds_list = [ds.split("\n")[0] for ds in lines]
...
root_path = os.path.join(superds_path, "inputs/brain_imaging_ds")
...
for ds in chunk_toget:
    ...
    try:
        ...
        with open("/ocean/projects/asc170022p/tighu/t1_error_files/T1exception-{}.txt".format(st), "at") as fp:
            fp.writelines(ds+str(e))


    # find fields to get in dataset
    fields_toget = glob.glob(path + "/" + "20252_[0-9]*_[0-9]")
    ...
    if fields_toget:
        ...
        with open("/ocean/projects/asc170022p/tighu/t1_error_files/T1data_missing-{}.txt".format(st), "at") as fp:
            fp.writelines(f"{ds}\n")
